...
func _collect_stack_tree_frames(tree: Tree) -> Array:
	...
	while item:
		frames.append(_build_frame_from_item(item, tree.columns, fallback_index))
		fallback_index += 1
		item = item.get_next()
...
func _build_frame_from_item(item: TreeItem, col_count: int, fallback_idx: int) -> Dictionary:
	var columns: Array = []
